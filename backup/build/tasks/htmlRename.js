...
function rulesMatch(result) {
    var terms = result.split(/['"]/);

    if (terms.length === 3) {
        return terms[1];
    }
    else if (/^\s*require/ig.test(result)) {
        return result
            .replace(/require\s*?\(/, '')
            .replace(/[\]\[]/ig,"");
    }
...
function clearEmptyLabel(string) {
    // 删除base下的manifest标签
...
function replaceContent(pathName, string) {
        ...
        function (reg, key) {
            string = string.replace(reg, function (value) {
                ...
                if (key == "require_to_url" ) {
                    if (util.isLocal()
                        || biz.loadImage.isWhiteList(pathName, fileName)
                    ) {
